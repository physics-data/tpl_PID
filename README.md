...
我们将这些参数进行分割，得到运行以下三个程序所需要用到的输入文件：`im_energy_depoisition.txt`, `im_light_spread.txt` 和 `im_responce.txt`。
...
读入 `energy_deposition.txt`，输出 `num_light.csv`。
...
读入 `light_spread.txt`和上一步的输出 `num_light.csv`，输出 `spread.csv`。
...
读入 `response.txt` 和上一步的输出 `spread.csv`，输出 `waveform.h5`。
