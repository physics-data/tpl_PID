...
读入 `im_light_spread.txt`和上一步的输出`num_light.csv`，输出 `spread.csv`。
...
读入 `im_responce.txt`和上一步的输出`spread.csv`，输出 `waveform.h5`。
...
根据入射粒子的类型和能量，计算带电粒子的电离能量损失（Bethe 公式描述），以及发射光子的数量。这里我们做一个简化，认为带电粒子在运动过程中只在探测器球心出发生一次能量沉积，该粒子的沉积能量的长度取 1ns，且作为点源发射各向同性的光子。同时，你还需要判断该带电粒子是否发生切伦科夫辐射，并计算切伦科夫辐射的发射光子数，如果不发生切伦科夫辐射，则将发射切伦科夫光子数输出为 0。这里需要区分发射光子的快慢成分。经过该步骤应该得到一个包括以下内容的输出文件`num_light.csv`（保留两位有效数字）：
...
读入 `num_light.csv`，输出 `spread.csv`。
