...
第三步：模拟光子经过光电倍增管放大，得到输出的波形（完成`response.py`）。该部分需要输出得到的包含两个输出波形图像的pdf文件（pic_responce.pdf）及两个波形的数据（alpha.csv和beta.csv），这里的数据文件（.csv，要求包括时间信息和对应的电压大小，时间长度为1-1029ns）。
