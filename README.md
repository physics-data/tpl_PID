...
根据入射粒子的类型和能量，计算带电粒子的电离能量损失（Bethe 公式描述），以及发射光子的数量。这里我们做一个简化，认为带电粒子在运动过程中只在探测器球心出发生一次能量沉积，该粒子的沉积能量的长度取 1um，且作为点源发射各向同性的光子（这里，简单的认为，alpha粒子和beta粒子沉积的能量全部用于发射光子，同时，这里的模型简化认为，两种粒子均只在这1um的范围内损失能量，且损失的过程瞬间完成）。
